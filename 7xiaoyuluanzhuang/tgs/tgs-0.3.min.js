...
function getQueryString() {
	for (var a, b = {}, c = location.search.substring(1), d = /([^&=]+)=([^&]*)/g; a = d.exec(c); )
...
"document" in self && !("classList" in document.createElement("_") && "classList" in document.createElementNS("/www.w3.org/2000/svg", "svg")) && !function (a) {
	"use strict";
	if ("Element" in a) {
		var b = "classList",
		c = "prototype",
...
(self);
var TGS = TGS || {}, merge = function (a, b) {
	b = b || {},
	a = a || {};
	for (var c in b)
		a[c] = b[c];
...
(), TGS.PartnerBridge = function () {
	...
	share : function (a, b) {
		...
		if (f > e && (i = Math.round(f / 2 - e / 2)), "twitter" == a) {
			...
			if ("string" != typeof window.location.host || -1 === window.location.host.indexOf("tresensa.com")) {
				var j = ["https://www.facebook.com/dialog/feed?", "app_id=", this._mFacebookAppID, "&display=popup&caption=", encodeURIComponent(b.msg), "&link=", encodeURIComponent(b.url), "&redirect_uri=", encodeURIComponent("")].join("");
	...
	masterInitialize : function () {
		...
		this._mFacebookServicesAllowed && ("undefined" != typeof FB ? (TGS.Debug.Log(TGS.Debug.LOG_INFO, "detected Facebook library..."), this.onFacebookLibraryLoaded()) : (TGS.Debug.Log(TGS.Debug.LOG_INFO, "loading Facebook library..."), head.js(("https:" === document.location.protocol ? "https:" : "http:") + "", this.onFacebookLibraryLoaded.bind(this)))),
		this.initialize()
	},
	initialize : function () {
	...
	initFacebook : function () {
		if (TGS.Debug.Log(TGS.Debug.LOG_VERBOSE, "PartnerBridge.initFacebook called"), !this._mFacebookInitialized) {
			if (!this._mFacebookAppID)
				return void(null !== TGS._sGamePartnerInfo && TGS.Debug.Log(TGS.Debug.LOG_ERROR, "cannot initialize Facebook without a Facebook app id (connection problem or the id was not defined in the TGS db)"));
	...
	userLoggedIn : function (a, b) {
		...
		if (c && this._mLoggedIn)
			return void(b === c ? TGS.Debug.Log(TGS.Debug.LOG_WARNING, "PartnerBridge.userLoggedIn: user " + b + " is already logged in") : (TGS.Debug.Log(TGS.Debug.LOG_ERROR, "PartnerBridge.userLoggedIn: there is already a logged in user"), TGS._LoginFailed(a)));
	...
	advertising_injectInterstitial : function (a) {
		...
		console.log(q.closeCallback && q.closeCallback.call());
	},
	...
	if (!(TGS._sPartnerID && TGS._sGameID && TGS._sUserID && TGS._sPartnerBridge && TGS._sPartnerBridge.isReady()))
		return TGS.Debug.Log(TGS.Debug.LOG_ERROR, "TGS.DataStore.ReloadGameData - TGS is not ready yet"), void(b && b.call(this, "TGS was not ready"));
	...
	if (!TGS._sPartnerBridge.supportsDatastore())
		return void TGS.Debug.Log(TGS.Debug.LOG_WARNING, "datastore operations are not available on this distribution partner");
	if (!(TGS._sPartnerID && TGS._sGameID && TGS._sUserID && TGS._sPartnerBridge && TGS._sPartnerBridge.isReady()))
		return TGS.Debug.Log(TGS.Debug.LOG_ERROR, "TGS.DataStore.ReloadGameData - TGS is not ready yet"), void(a.onFailure && a.onFailure.call(this, "TGS was not ready"));
	...
	if (!b || "undefined" == typeof b.code)
		return void TGS.DataStore._ErrorCallback(a, "response object is malformed", b);
	if (0 !== b.code)
		return void TGS.DataStore._ErrorCallback(a, "internal request error code: " + b.code, b);
	...
	if (!TGS._sPartnerBridge.supportsDatastore())
		return void TGS.Debug.Log(TGS.Debug.LOG_WARNING, "datastore operations are not available on this distribution partner");
	if (!(TGS._sPartnerID && TGS._sGameID && TGS._sUserID && TGS._sPartnerBridge && TGS._sPartnerBridge.isReady()))
		return TGS.Debug.Log(TGS.Debug.LOG_ERROR, "TGS.DataStore.ReloadGameData - TGS is not ready yet"), void(d && d.call(this, "TGS was not ready"));
	...
	if (0 !== b.code)
		return void TGS.DataStore._ErrorCallback(a, "internal request error code: " + b.code, b);
	if (!a.noLocalData && TGS.DataStore._sSaveToLocalStorage)
		return void TGS.Debug.Log(TGS.Debug.LOG_INFO, "remote DataStore request was successful");
	var c = null;
	...
	if (!TGS._sPartnerBridge.supportsDatastore())
		return void TGS.Debug.Log(TGS.Debug.LOG_WARNING, "datastore operations are not available on this distribution partner");
	if (!TGS.IsReady())
		return void TGS.Debug.Log(TGS.Debug.LOG_WARNING, "DataStore tried to save value for '" + a + "' before TGS was ready");
	...
	if (!TGS._sPartnerBridge.supportsDatastore())
		return void TGS.Debug.Log(TGS.Debug.LOG_WARNING, "datastore operations are not available on this distribution partner");
	if (!TGS.IsReady())
		return void TGS.Debug.Log(TGS.Debug.LOG_WARNING, "DataStore tried to save values before TGS was ready");
	...
	if (b = "undefined" == typeof b ? !1 : b, !TGS._sPartnerBridge.supportsDatastore())
		return void TGS.Debug.Log(TGS.Debug.LOG_WARNING, "datastore operations are not available on this distribution partner");
	if (!TGS.IsReady())
		return void TGS.Debug.Log(TGS.Debug.LOG_WARNING, "DataStore tried to save values before TGS was ready");
	for (var c in a)
		a.hasOwnProperty(c) && (TGS.DataStore.GameData[c] = a[c].toString());
	if (b) {
	...
	for (var a = 0; a < TGS.Microtransactions._sIAPProducts.length; a++)
		if (!TGS.Microtransactions._sIAPProducts[a].consumable)
			return !0;
	return !1
}, TGS.Microtransactions.PurchaseProduct = function (a) {
	if (!TGS._sPartnerBridge.supportsMicrotransactions())
		return void TGS.Microtransactions.RedirectToIAPPartner();
	if (null !== TGS.Microtransactions._PendingRequest)
		return void TGS.Debug.Log(TGS.Debug.LOG_WARNING, "purchase request ignored - one is already pending");
	var b = a.onFailure;
	TGS.Microtransactions.UseTGSErrorMessages && (b = TGS.Microtransactions._PurchaseFailedMessage.bind(this, a.onFailure));
	var c = !0,
	d = "";
	if ("object" != typeof a ? (c = !1, d += "params ") : ("undefined" == typeof a.productID && (c = !1, d += "productID "), "object" != typeof a.gameDataUpdates && (c = !1, d += "gameDataUpdates ")), !c)
		return TGS.Debug.Log(TGS.Debug.LOG_ERROR, "Invalid values sent to TGS.Microtransactions.PurchaseProduct for the following params: " + d), void(a && a.onFailure && b.call(this, a.productID, TGS.Microtransactions.DefaultPurchaseFailedMessage));
	var e = TGS.Microtransactions.GetIAPProduct(a.productID);
	if (!e)
		return TGS.Debug.Log(TGS.Debug.LOG_ERROR, "Unknown IAP product: " + a.productID), void(a.onFailure && b.call(this, a.productID, TGS.Microtransactions.DefaultPurchaseFailedMessage));
	if (!e.consumable && null === TGS.Microtransactions.RestorePurchaseCallback)
	...
	if (!TGS.LoggedIn())
		return void TGS.OverlayMessage({
	...
	if (!TGS.IsReady())
		return TGS.Debug.Log(TGS.Debug.LOG_ERROR, "tried to purchase product " + f + " but TGS was not ready"), void(h && h.call(this, f, TGS.Microtransactions.DefaultPurchaseFailedMessage));
	...
	if (TGS.Debug.Log(TGS.Debug.LOG_VERBOSE, "purchase request id received: " + JSON.stringify(b)), !b || "undefined" == typeof b.code)
		return TGS.Debug.Log(TGS.Debug.LOG_INFO, "purchase request response was corrupt"), void TGS.Microtransactions._LoggingPurchaseFailed(a);
	if (0 !== b.code)
		return TGS.Debug.Log(TGS.Debug.LOG_INFO, "purchase request response contained error code: " + b.code), void TGS.Microtransactions._LoggingPurchaseFailed(a);
	var c = parseInt(b.id);
	...
	if (b.freeIAP) {
		...
		b.offerValueExchange ? (TGS.Debug.Log(TGS.Debug.LOG_INFO, "initiating value exchange ad for item '" + a.item.id + "'..."), TGS.Microtransactions._PurchaseOverlay.style.display = "none", TGS.Advertisement.DisplayValueExchangeAd({
	...
	if (console.log(JSON.stringify(a)), TGS.Debug.Log(TGS.Debug.LOG_INFO, "purchase successful for item " + a.item.id), TGS.Microtransactions._SavePurchaseLocally(a.gameDataUpdates), TGS.Debug.Log(TGS.Debug.LOG_VERBOSE, "local game data has now been updated"), TGS._sPartnerBridge.serverHandlesMicrotransactionResponse())
		TGS.Debug.Log(TGS.Debug.LOG_VERBOSE, "server has already logged transaction"), a.onSuccess && a.onSuccess.call(this, a.item.id);
	...
	if (TGS.Debug.Log(TGS.Debug.LOG_ERROR, "purchase failed for item " + a.item.id + ", reason: " + a.internalErrorMessage), TGS._sPartnerBridge.serverHandlesMicrotransactionResponse())
		TGS.Debug.Log(TGS.Debug.LOG_VERBOSE, "server has already logged transaction"), a.onFailure && a.onFailure.call(this, a.item.id, a.userErrorMessage);
	else if (TGS._sPartnerBridge.logTransactionBeforePartnerRequest()) {
	...
	window.open("/fortumo.com/mobile_payments/0d8f60636b898652b94ee978ee2fc43a?cuid=" + TGS._sPartnerID + TGS._sUserID, "_self")
			...
			if (null != a[b]) {
				...
				if (this.versionSearchString = a[b].versionSearch || a[b].identity, c) {
					if (-1 !== c.indexOf(a[b].subString))
						return "Mozilla" === a[b].identity && navigator.userAgent.match(/Trident\/7\./) ? "Explorer" : a[b].identity
				} else if (d)
					return a[b].identity
			}
	},
...
}, TGS.BrowserDetect.init(), function () {
	...
	TGS.Leaderboard.getLabel = function () {
		var a = !!parseInt(TGS._sPartnerBridge._mUseUserInfoInSLB || -1 !== window.location.host.indexOf("tresensa.com") && TGS._sPartnerBridge.loggedIntoFacebook() ? 0 : TGS.localStorage.getItem(TGS._sLSKeys.TGS_pending_score));
		return TGS._sPartnerBridge.leaderboardLabel(a)
	},
	...
	TGS.Leaderboard.Show = function (a) {
		return TGS.LoggedIn() ? (TGS._sPartnerBridge.implementsLeaderboard() ? TGS._sPartnerBridge.showLeaderboard(a) : TGS.Leaderboard._ShowSLB(a), void TGS.Analytics.logScreen("leaderboard")) : void TGS.LoginUser({
	...
	TGS.Leaderboard._SubmitScoreSLB = function (a, b) {
		...
		if (!TGS._sPartnerBridge._mUseUserInfoInSLB) {
			...
			f = e && e.descending === !1;
			if (-1 === window.location.host.indexOf("tresensa.com"))
				return (f && a.score < TGS.localStorage.getItem(TGS._sLSKeys.TGS_pending_score) || !f && a.score > TGS.localStorage.getItem(TGS._sLSKeys.TGS_pending_score)) && TGS.localStorage.setItem(TGS._sLSKeys.TGS_pending_score, a.score), TGS.Debug.Log(TGS.Debug.LOG_WARNING, "user is not logged in/authorized on Facebook - saving score"), void(a.onSuccess && a.onSuccess.call(this, a.score, a.leaderboardID));
			if (!TGS._sPartnerBridge.loggedIntoFacebook())
				return b ? void TGS._sPartnerBridge.loginToFacebook(TGS._sPartnerBridge.promptForFacebookAuthorization.bind(TGS._sPartnerBridge), TGS.Leaderboard._SubmitScoreSLB.bind(this, a)) : ((f && a.score < TGS.localStorage.getItem(TGS._sLSKeys.TGS_pending_score) || !f && a.score > TGS.localStorage.getItem(TGS._sLSKeys.TGS_pending_score)) && TGS.localStorage.setItem(TGS._sLSKeys.TGS_pending_score, a.score), void TGS.Debug.Log(TGS.Debug.LOG_WARNING, "user is not logged in/authorized on Facebook - saving score"));
			c = TGS._sPartnerBridge._mFacebookUserID,
			d = "A0003"
		}
		var g = !0,
		h = "";
		if ("object" != typeof a ? (g = !1, h += "params ") : "number" != typeof a.score && (g = !1, h += "score "), !g)
			return TGS.Debug.Log(TGS.Debug.LOG_ERROR, "Invalid values sent to TGS.Leaderboard.SubmitScore for the following params: " + h), void(a && a.onFailure && a.onFailure.call(this, a.score, a.leaderboardID));
		a.leaderboardID = "number" == typeof a.leaderboardID ? a.leaderboardID : 0;
		var i = "function" == typeof a.onFailure ? a.onFailure : null,
		j = "",
		k = "";
		TGS._sPartnerBridge._mUseUserInfoInSLB ? (j += ',"name":' + (TGS._sPartnerBridge._mUsername ? '"' + TGS._sPartnerBridge._mUsername + '"' : "null"), j += ',"image_url":' + (TGS._sPartnerBridge._mAvatarURL ? '"' + TGS._sPartnerBridge._mAvatarURL + '"' : "null"), j += ',"location":' + (TGS._sPartnerBridge._mUserLocation ? '"' + TGS._sPartnerBridge._mUserLocation + '"' : "null")) : k += ',"facebook_access_token":' + (TGS._sPartnerBridge._mFacebookAccessToken ? '"' + TGS._sPartnerBridge._mFacebookAccessToken + '"' : "null");
		var l = '{"game":"' + TGS._sGameID + '","partner":"' + d + '","leaderboard":"' + a.leaderboardID + '","user":"' + c + '","score":' + a.score + k + j + "}",
		m = new TGS.LeaderboardRequest;
		m.type = "add",
		m.params = a,
		m.onSuccess = a.onSuccess,
		m.onFailure = i,
		TGS.Debug.Log(TGS.Debug.LOG_INFO, "submitting new score of " + a.score + " to server..."),
		TGS.SendMessage("leaderboard/score", "add", l, TGS.Leaderboard._SuccessCallback.bind(this, m), TGS.Leaderboard._ErrorCallback.bind(this, m))
	},
	TGS.Leaderboard._ShowSLB = function (a) {
		var d,
		e = (document.getElementsByTagName("base"), document.createElement("a"));
		e.href = "",
		d = e.href;
		var f = a.score,
		...
		var j = {
			...
			score : a.score || parseInt(TGS.localStorage.getItem(TGS._sLSKeys.TGS_pending_score) || 0),
		...
		if (TGS.localStorage.setItem(TGS._sLSKeys.TGS_pending_score, 0), TGS._sPartnerBridge._mUseUserInfoInSLB)
			j.userInfo = {
				id : TGS._sUserID,
		...
		else if (-1 === window.location.host.indexOf("tresensa.com"))
			j.partner = "A0003";
		else {
			if (!TGS._sPartnerBridge.loggedIntoFacebook())
				return void TGS._sPartnerBridge.loginToFacebook(TGS._sPartnerBridge.promptForFacebookAuthorization.bind(TGS._sPartnerBridge), TGS.Leaderboard._ShowSLB.bind(this, a));
			j.partner = "A0003",
			j.userInfo = {
		...
		if (TGS.Utils.isTGEGame() || a.canvasScaled)
		...
		s = function (a) {
			if (!a.origin.match("^https?://(qa-)?leaderboard.tresensa.com$"))
				return void TGS.Debug.Log(TGS.Debug.LOG_WARNING, "Recieved window message: " + a.data + " from unexpected origin: " + a.origin);
...
(), function () {
	...
	TGS.Advertisement.prototype = {
		...
		_handleWindowMessage : function (a) {
			if (!a.origin.match("^https?://adserver.adtechus.com$"))
				return void TGS.Debug.Log(TGS.Debug.LOG_WARNING, "Received window message: " + a.data + " from unexpected origin: " + a.origin);
			var b = JSON.parse(a.data);
...
(), function () {
	...
	b = [/.+/i],
	c = {
		game : ["load", "begin", "pause", "resume", "end", "adrequest", "win", "lose", "restart", "login success", "login failure", "login cancel", "logout"],
	...
	i = function (a, b, f, i) {
		...
		var l = i.settings || {};
		try {
			if (a = (a || "").toLowerCase(), b = (b || "").toLowerCase(), j = c[a], !j)
				return void TGS.Debug.Log(TGS.Debug.LOG_ERROR, "TGS.Analytics.trackEvent called with invalid category '" + a);
			if (!j.some(d(b)))
				return void TGS.Debug.Log(TGS.Debug.LOG_ERROR, "TGS.Analytics.trackEvent called with invalid name '" + b + "' for category '" + a);
			if (g)
	...
	TGS.Analytics = {
		_init : function (a) {
			...
			j;
			if (!g) {
				for (TGS.Debug.Log(TGS.Debug.LOG_VERBOSE, "TGS.Analytics.init called with config: " + JSON.stringify(a)), f = a, b = TGS._sPartnerBridge.analytics_getProviders() || [], TGS.Debug.Log(TGS.Debug.LOG_VERBOSE, "TGS.Analytics.init got " + b.length + " providers from partner bridge"), c = 0; c < b.length; c++)
					try {
...
(), function () {
	...
	!function (b) {
		...
		h = h.MD5 = j.extend({
				...
				_doFinalize : function () {
					...
					for (c[(e + 64 >>> 9 << 4) + 15] = 16711935 & (f << 8 | f >>> 24) | 4278255360 & (f << 24 | f >>> 8), c[(e + 64 >>> 9 << 4) + 14] = 16711935 & (d << 8 | d >>> 24) | 4278255360 & (d << 24 | d >>> 8), a.sigBytes = 4 * (c.length + 1), this._process(), a = this._hash, c = a.words, d = 0; 4 > d; d++)
	...
	TGS.Analytics.GameAnalyticsProvider.prototype = {
		...
		init : function (a) {
			...
			this._mBuild = b.join(" "),
			this._mSessionId = "xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx".replace(/[xy]/g, function (a) {
					var b = 16 * Math.random() | 0,
					c = "x" == a ? b : 3 & b | 8;
					return c.toString(16)
				})
		...
		_apiCall : function (b, c) {
			...
			TGS.CORSRequest(f, g, !1, !0, function () {}, function () {}, {
				Authorization : i
			})
		},
...
(), function () {
	...
	TGS.Analytics.GoogleAnalyticsProvider.prototype = {
		...
		trackEvent : function (a, b, c, d) {
			...
			this._mConfig.STUDIO_ONLY || "ads" == a || "achievement" == a || "custom" == a || _gaTreSensa("TGSTracker.send", "event", a, b, this._mConfig.LABEL || TGS._sGameID, c, f),
	...
	!function (a, b) {
		if (!b.__SV) {
			...
			b.init = function (a, c, d) {
				...
				}, e = "disable track track_pageview track_links track_forms register register_once alias unregister identify name_tag set_config people.set people.set_once people.increment people.append people.track_charge people.clear_charges people.delete_user".split(" "), f = 0; f < e.length; f++)
					g(h, e[f]);
				b._i.push([a, c, d])
			},
			b.__SV = 1.2,
...
(), function () {
	...
	TGS.Analytics.NativeGoogleAnalyticsProvider.prototype = {
		...
		configName : "GoogleAnalytics",
		init : function (a) {
			if (TGS.Debug.Log(TGS.Debug.LOG_VERBOSE, "NativeGoogleAnalyticsProvider.init called: " + JSON.stringify(a)), this._mConfig = a, this._gaPlugin = window.plugins.gaPlugin, !a.NATIVE_ID)
				return TGS.Debug.Log(TGS.Debug.LOG_ERROR, "CordovaConfig.GoogleAnalytics.NATIVE_ID not set, disabling analytics"), !1;
...
(), function () {
	var a = function (a, b, c, d) {
		...
		if (e.classList.add("tgs-btn"), c)
			for (var f = c.split(" "), g = f.length - 1; g >= 0; g--)
				e.classList.add(f[g]);
		var h = "",
		i = !1;
	...
	TGS.Widget = function (c) {
		...
		if (!c.disableShare)
			for (var q = {
					...
					msg : c.shareMessage,
					url : c.shareUrl,
					image : c.shareImageUrl
				}, r = TGS.Social.GetShareServices(2), s = 0; s < r.length; s++) {
				var t = r[s];
				this.buttonContainer.appendChild(a(t.label, TGS.Social.Share.bind(null, t.id, q), t.id))
			}
		if (!c.disableMoreGames && TGS.MoreGamesURL() && this.buttonContainer.appendChild(a("More Games", TGS.ShowMoreGames, "moregames")), !c.disableLeaderboard && TGS.Leaderboard.isSupported() && Object.keys(TGS.Leaderboard.GetLeaderboardDescriptors()).length > 0) {
			...
			this.buttonContainer.appendChild(a(TGS.Leaderboard.getLabel(), u, "leaderboard"))
		}
		var v = function () {
			if (m > 1) {
...
(), function () {
	...
	TGS.SendMessage = function (a, b, c, d, e) {
		if (!TGS._sOnline)
			return void(e && e("TGS is offline"));
	...
	TGS.CORSRequest = function (a, b, c, d, e, f, g) {
		...
		try {
			if (window.XMLHttpRequest) {
				if (h = new XMLHttpRequest, "withCredentials" in h)
					h.open(i, a, !0), l(h, g), h.send(b);
				...
				h = new ActiveXObject("Microsoft.XMLHTTP"), h.open(i, a, !0), l(h, g), h.send(b)
		} catch (o) {
			f && f("server failed to respond", o)
		}
		try {
			k ? (h.timeout = 1e4, h.onerror = function () {}, h.ontimeout = function () {}, h.onprogress = function () {}, h.onload = function () {
				if (!d)
					return void e(h.responseText);
				try {
				...
				if (j) {
					...
					f && (TGS.Debug.Log(TGS.Debug.LOG_WARNING, "server request returned bad data"), f("server request returned bad data", null))
			}) : h.onreadystatechange = function () {
				if (4 == h.readyState)
					if (200 != h.status)
						f && (TGS.Debug.Log(TGS.Debug.LOG_WARNING, "the page returned error code " + h.status + " message: " + h.statusText), f("the page returned error code " + h.status, h));
					else {
						if (!d)
							return void e(h.responseText);
						try {
	...
	TGS.Init = function (a, b) {
		if (1 == arguments.length ? (TGS._sConfig = a, a = TGS._sConfig.GAME_ID, b = TGS._sConfig.DST_ID) : (window.GameConfig.DST_ID = TGL.getDistributionPartner(), TGS._sConfig = window.GameConfig), TGS._TestLocalStorage(), "string" != typeof a || a.length < 1)
			return void TGS.Debug.Log(TGS.Debug.LOG_ERROR, "a valid game id was not specified");
		TGS._sGameID = a;
	...
	TGS._injectCSS = function () {
		var a = window.TGL ? TGL.SDKBaseURL + "tgs/" : "./tgs/",
	...
	TGS._LoadAdapterLibraries = function () {
		...
		b = [],
		c = window.TGL ? TGL.SDKBaseURL + "tgs/" : "./tgs/",
		...
		a && a.fb !== !0 || b.push(("https:" === document.location.protocol ? "https:" : "http:") + ""),
	...
	TGS._CreatePartnerBridge = function () {
		if (TGS.Adapters[TGS._sPartnerID])
			try {
			...
			TGS.Debug.Log(TGS.Debug.LOG_INFO, "creating generic adapter..."), TGS._sPartnerBridge = new TGS.Adapters.GenericAdapter;
	...
	TGS.onGamePartnerInfoError = function (a, b) {
		...
		c ? (TGS.Debug.Log(TGS.Debug.LOG_INFO, "using a local copy of the game-partner info"), b = JSON.parse(c), TGS.Debug.Log(TGS.Debug.LOG_VERBOSE, "game-partner info: " + JSON.stringify(b)), TGS._sGamePartnerInfo = b.partner_values, TGS.Microtransactions._sIAPProducts = b.iap_products, TGS.Microtransactions._sAppURLs = b.app_urls, TGS.Leaderboard._sLeaderboardDescriptors = b.leaderboards) : (TGS.Debug.Log(TGS.Debug.LOG_INFO, "no local copy of the game-partner info is available"), TGS.OverlayMessage({
			...
			}), TGS._sGamePartnerInfo = {}, TGS.Microtransactions._sIAPProducts = [], TGS.Microtransactions._sAppURLs = [], TGS.Leaderboard._sLeaderboardDescriptors = []),
	...
	TGS.AddRequiredImagesToAssetList = function (a) {
		if (!window.TGE)
			return TGS.Debug.Log(TGS.Debug.LOG_ERROR, "you must be using the TreSensa TGE library to use TGS.AddRequiredImagesToAssetList"), null;
	...
	TGS.CreateLoginWidget = function (a) {
		if (!window.TGE)
			return TGS.Debug.Log(TGS.Debug.LOG_ERROR, "you must be using the TreSensa TGE library to use TGS.CreateLoginWidget"), null;
		...
		if ("A0001" !== TGS._sPartnerID && "A0002" !== TGS._sPartnerID && "A0030" !== TGS._sPartnerID || !TGS.Microtransactions.HasNonConsumables())
			if (TGS.AutoLogin() || !TGS._sPartnerBridge.supportsLogout() && TGS.LoggedIn() || !TGS.LoginIcon());
				...
				c.addEventListener("tgs_login_changed", function (a) {
					...
					!TGS._sPartnerBridge.supportsLogout() && TGS.LoggedIn() && (b.enabled = !1, b.visible = !1, c = null),
					b.setImage(c)
				}),
				b.addChild(c)
			}
		else
			b.addChild((new TGE.Button).setup({
	...
	TGS.CreateMoreGamesButton = function (a) {
		if (!window.TGE)
			return TGS.Debug.Log(TGS.Debug.LOG_ERROR, "you must be using the TreSensa TGE library to use TGS.CreateMoreGamesButton"), null;
		var b = new TGE.DisplayObjectContainer;
		if (TGS.MoreGamesURL()) {
			var c = (new TGE.Button).setup({
			...
			e = 0,
			f = 1,
			g = TGE.Game.GetInstance().stage;
			-1 !== a.indexOf("top") && (e = 40),
	...
	TGS.OverlayMessage = function (a) {
		if (!a || !a.message)
			return void TGS.Debug.Log(TGS.Debug.LOG_ERROR, "no message specified in TGS.OverlayMessage call");
...
(), TGS.Adapters.GenericAdapter = function () {
	...
	this._mMoreGamesURL = "/www.mobilewebarcade.com/"
