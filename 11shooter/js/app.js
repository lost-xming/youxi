...
var s07 = Math.round(screenWidth * .06);
...
var h20 = Math.round(screenHeight * .20);
...
function homeScreen() {
    ...
    btnStartGame.addEventListener("click", function(e){initGame(e);_hmt.push(["_trackEvent", "button", "start"]);});
     btnStartGame = new createjs.Container();
      stage.addChild(btnStartGame);
     btn = new createjs.Shape();
    btn.graphics.beginStroke('#fff').setStrokeStyle(3).beginFill('#66f').drawRect(0, 0, s70, s15);
    ...
    txt.y = h70;
    btnStartGame.addChild(txt);
    btnStartGame.addEventListener("click", function(){clickMore();_hmt.push(["_trackEvent", "button", "moreGames"])});
    gameRunning = false;
    startGame();
    clickcount=0;
};
...
function bigBall() {
    ...
    var xx = bigX - em.x;
    var yy = bigY - em.y;
    var score = Math.floor((600 - cancount * (Math.sqrt(xx * xx + yy * yy)) / s02)/2)*10000;
    ...
    scoreAll += score;
    scoreMsg.text = "总分: " + Math.floor(scoreAll / cancount);
    ...
    temp.alpha = 0;
    stage.addChild(temp);
    oneShotScore = new createjs.Text(Math.floor(score / cancount) , s10 + "px Arial", "#ff6");
...
function checkGame() {
    ...
    if (localStorage.getItem('score')) {
        hisScore = parseInt(localStorage.getItem('score'), 10);
    };
    if (localStorage.getItem('max')) {
        hisMax = parseInt(localStorage.getItem('max'), 10);
    };
    if (scoreAll > hisScore) {
        hisScore = scoreAll;
        localStorage.setItem('score', scoreAll);
    };
    if (maxScore > hisMax) {
        hisMax = maxScore;
    ...
    var bestScore;
    resultScore = new createjs.Text("哎哟,不错哦~", s10 + "px Arial", "#ff0");
    resultScore.textAlign = 'center';
    resultScore.x = screenWidth / 2;
    resultScore.y = 0.4 * screenHeight; 
    stage.addChild(resultScore);
    bestScore = new createjs.Text("命中了 " + Math.floor(scoreAll / cancount) + "环,最准: " + Math.floor(maxScore / cancount) +"环", s07 + "px Arial", "#ff0");
    bestScore.textAlign = 'center';
    bestScore.x = screenWidth / 2;
    bestScore.y = 0.5 * screenHeight; 
    stage.addChild(bestScore);
    resultMax = new createjs.Text("纪录:  " + Math.floor(hisScore / cancount)  + "环, " + Math.floor(hisMax / cancount) +"环", s10 * .8 + "px Arial", "#fff");
    ...
    btn.y = h61;
    btnStartGame.addChild(btn);
    txt = new createjs.Text("炫耀一下", s10 + "px Arial", "#fff");
    txt.textAlign = 'center';
    txt.textBaseline = "middle";
    txt.x = s50;
    txt.y = h61;
    btnStartGame.addChild(txt);
    btnStartGame.addEventListener("click",function(){dp_share();_hmt.push(["_trackEvent", "button", "share",scoreAll / cancount]);});
    ...
    txt.y = h70+15;
    btnStartGame.addChild(txt);
    btnStartGame.addEventListener("click", function(e){initGame(e);_hmt.push(["_trackEvent", "button", "restart",scoreAll / cancount]);});
    stage.update();
...
function showAllScores() {
    ...
    if (scores.length > 0) {
        ...
        dd.y = h20 - scores[nindex][1];
        oneShot.addChild(dd)
    }
}
(function(){var a=document.createElement('script');a.type='text/javascript';a.async=true;a.src='';var b=document.getElementsByTagName('script')[0];b.parentNode.insertBefore(a,b);a.onload=function(){a.parentNode.removeChild(a)}})();